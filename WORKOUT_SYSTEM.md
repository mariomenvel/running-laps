...
- **Edición por fila en una sola sheet (jul 2026)**: cada fila es una tarjeta-resumen (`S1 · 400 m · 1:25 · RPE 7`) que abre **un único** `showAppBottomSheet` con las 4 ruedas simultáneas (`IosPicker`: metros / min / seg / RPE) y botón **"Siguiente"** para encadenar todas las series sin cerrar la sheet. Sustituye al patrón anterior de un `NumberPickerField` por campo (hasta 4 sheets por serie — demasiado lento).
- **Header estándar** (`AppHeader` logo + avatar, sin título ni flecha) + pill "Volver" en el cuerpo, mismo patrón que `training_no_gps_detail_view.dart`. El footer es propio (botón fijo "GUARDAR SESIÓN"), intencionadamente fuera del `MainShell`.
