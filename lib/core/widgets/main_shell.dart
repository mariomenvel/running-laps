...
class _MainShellState extends State<MainShell> {
  ...
  final ValueNotifier<PreExecutionShellParams?> _preExecutionNotifier =
      ValueNotifier(null);
  ...
  late final List<Widget> _screens = [
    ...
    ValueListenableBuilder<PreExecutionShellParams?>(
      valueListenable: _preExecutionNotifier,
      builder: (_, p, __) => p != null
          ? PreExecutionScreen(
              key: ValueKey('pre_exec_${p.session.id}'),
              session: p.session,
              athleteSession: p.athleteSession,
            )
          : const SizedBox.shrink(),
    ),
  ];
    ...
    switch (index) {
      ...
      case 16:
        if (params is PreExecutionShellParams) {
          _preExecutionNotifier.value = params;
        }
    }
    setState(() {
      _previousTabIndex = _tabIndex;
