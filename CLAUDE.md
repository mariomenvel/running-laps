...
5. **Rediseño de `AccountSettingsView`** (decidido 26 jul 2026, sin empezar) — la pantalla de ajustes actual (`profile/views/account_settings_view.dart`, slot 8 del shell) es **la única** que hay, pero su UI es antigua. Del contenido actual solo se llevan a producto **tres funciones: cambiar nombre, borrar cuenta y GPS por defecto**, que irán integradas en el flujo normal (otra vista o el propio menú de Perfil). La pantalla de ajustes se rehará con el sistema visual actual del manual de identidad, porque es donde vivirá la **gestión de la suscripción** (ver `docs/MONETIZATION_ARCHITECTURE.md`). Nota: la lógica de datos ya está fuera de la vista (`UserService.getBestMarkDistanceM/setBestMarkDistanceM`, `SettingsService`, `UserService.deleteAccount`), así que el rediseño no arrastra lógica.
   Bug abierto de esa misma pantalla: su `GradientBanner` ("Configuración de cuenta") **es invisible** — el banner pinta el texto siempre en blanco (`gradient_banner.dart`) y ahí recibe `accentColor: AppColors.surfaceOf(context)`, que en claro es casi blanco. Mismo fallo en `admin_panel_screen.dart:54`. Los demás usos pasan `brandSurface`/`restSurface` y se ven bien.
