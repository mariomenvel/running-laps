...
10. **`SessionTheme` (pantalla de sesión activa) incumple COLOR_SYSTEM.md entero** (encontrado 26 jul 2026, verificado en dispositivo — el botón "EMPEZAR"/"FINALIZAR CALENTAMIENTO" se ve teal). `lib/features/training/views/session_screens/shared/session_theme.dart` define una paleta hardcodeada distinta **por tipo de sesión** (terracota `intervals`, azul-verde `continuous` — literalmente `0xFF4A90A4`, el teal prohibido —, naranja/rojo `fartlek` en modo intenso, marrón/rojo `hills`, dorado/rojo `competition`, gris `free`), con degradados propios fuera de `AppColors`. Se usa en 8 ficheros (46 referencias): los 6 `session_screens/intra/*_screen.dart`, `rest_screen.dart` y `session_layout.dart` — es decir, toda la pantalla de calentamiento/bloque principal/vuelta a la calma/descanso durante un entreno.
    No es un descuido: es un sistema de diseño deliberado y extenso ("cada tipo de sesión tiene su propia personalidad visual"), así que no se ha tocado sin decisión de producto ni verificación visual en dispositivo para los 6 tipos. Si se decide alinearlo con el sistema de color: opciones son (a) migrar cada color a un token nuevo de `AppColors` manteniendo la idea de paleta por tipo, o (b) unificar a un único acento (`AppColors.brand`) como el resto de la app y eliminar `session_theme.dart` casi entero. Cualquiera de las dos exige repasar los 8 ficheros en el móvil, tipo por tipo.
