...
| `AppDialog` | `app_dialog.dart` | **Base de todos los diálogos.** Una sola cara en Android e iOS — ni Cupertino ni Material. Botones en fila que se apilan solos si las etiquetas no caben (medido con `TextPainter`, respetando el ajuste de fuente del sistema). Golden test en `test/widget/core/app_dialog_golden_test.dart`: `flutter test --update-goldens` para verlo sin dispositivo. |
| `showAppConfirmDialog` | `app_confirm_dialog.dart` | Diálogo de confirmación sobre `AppDialog`. Único modo permitido de preguntar "¿seguro?". `isDestructive: true` → botón rojo; `false` → morado brand. **Nunca** construir uno a mano con `showModalBottomSheet<bool>` — hay test que lo vigila. |
| `PickerSheetHeader` | `picker_sheet_header.dart` | Cabecera común de los sheets de selección: cancelar · título · confirmar. La comparten `number_picker_field`, `app_date_picker` y `wheel_sheets`, que antes tenían tres patrones y hasta palabras distintas ("Hecho" vs "Confirmar"). `onConfirm: null` lo deshabilita. |
...
| `FcZoneBars` | `fc_zone_bars.dart` | Reparto del tiempo por zona de FC en barras. Único punto donde se pinta: lo comparten el resumen post-entreno y el detalle del historial, y el cálculo (`FcAnalytics`) es el mismo que recibe el Coach IA. |
...
- **Frecuencia cardíaca** (jul 2026): el contexto lleva por entreno `fcAvg`/`fcPeak`, `zonesPct` (reparto del tiempo por zona), `efficiency` (m/min por pulsación), `decouplingPct` y `hrDriftBpm`, más `fcMax`/`fcRest` en el perfil. El cálculo es puro y vive **fuera** del Coach: `features/training/data/fc_analytics.dart` + `fc_session_stats.dart`, compartido con el resumen post-entreno y el historial vía `core/widgets/fc_zone_bars.dart` — app y Coach no pueden dar porcentajes distintos del mismo entreno. ⚠️ El reparto por zonas se pondera **por tiempo**, no por número de lecturas (una serie muestreada más densa se comía el porcentaje), y la FC media de sesión sale de las lecturas reales, no de promediar las `fcMedia` de cada serie. Detalle en `PREMIUM_AI_COACH.md` § Frecuencia cardíaca en el contexto.
...
**5. Escaneo BLE del pulsómetro — nunca filtrar por servicio en el escaneo**
No volver a poner `withServices: [0x180D]` en `scanForDevices` (`heart_rate_service.dart`): el filtro es nativo (ScanFilter/CoreBluetooth) y solo mira el paquete de advertising, que muchas bandas no incluyen el Heart Rate Service — resultado, escaneo eterno con cero resultados (jul 2026). Se escanea abierto y se filtra en Dart (`advertisesHeartRate` solo etiqueta y ordena, no descarta). Dos trampas más del mismo bug: el primer valor de `statusStream` es siempre `unknown` (esperar a un estado definitivo antes de escanear), y en Android ≤ 30 el escaneo BLE necesita la **ubicación del sistema encendida**. Y la causa nº1 que no está en el código: una banda BLE emparejada en los ajustes de Bluetooth del móvil deja de anunciarse — hay que quitarla de ahí.
...
5. **Pantalla de ajustes** — ✅ rehecha (26 jul 2026). `AccountSettingsView` (1.231 líneas) se sustituyó por `profile/views/settings_view.dart`, con **solo las tres funciones que van a producto**: cambiar nombre, borrar cuenta y GPS por defecto. Se retiraron por decisión de producto: estilo de tarjetas, alarmas por defecto, distancia de marca destacada y **cambiar contraseña** (⚠️ consecuencia: una cuenta de email/contraseña ya no puede cambiarla desde dentro de la app; queda el "olvidé mi contraseña" por email desde el login). Es la pantalla donde vivirá la **gestión de la suscripción** (`docs/MONETIZATION_ARCHITECTURE.md`).
   ...
   **Pendiente relacionado — panel de admin** (decidido 26 jul 2026): `admin/views/admin_panel_screen.dart` y sus tabs son legacy; la intención es **sacar esas métricas de la app a un panel web**. No invertir en su UI mientras tanto (por ejemplo, su `GradientBanner` con `accentColor: surfaceOf` sigue el mismo patrón que hacía invisible el de ajustes — arreglado en el widget, pero la pantalla en sí está para irse).
   ...
   ⚠️ **Y sus métricas globales ya están rotas** (visto 5 ago 2026): `AdminRepository.getGlobalStats()` hace `collectionGroup('trainings')` y, desde que `trainings` es `read: isOwner` (ago 2026, al quitar los rankings de grupo), Firestore lo deniega — el `try` interno se traga el error y el panel enseña ceros en vez de fallar. **No se arregla aflojando la regla**: el documento de entrenamiento lleva FC, dato del art. 9. Se arregla agregando en una Cloud Function con Admin SDK, que es lo que hará el panel web. Relacionado: `users/{uid}` conserva `read: isSignedIn()` (lectura cruzada de perfiles, con `birthDate`/`sex`/`fcMax` dentro) **solo** porque ese panel lo necesita — al migrarlo, cerrar la regla a `isOwner(uid) || isAdmin()`.
...
10. ~~**`SessionTheme` (pantalla de sesión activa) incumple COLOR_SYSTEM.md**~~ — ✅ cerrada (31 jul 2026), y al abrirla apareció algo peor detrás.

    **`session_theme.dart`** definía una paleta hardcodeada por tipo de sesión ("cada tipo tiene su propia personalidad visual"). Al medir el uso real, la mayor parte **no se renderizaba**: `accent()` y `heroMetricStyle()` no tenían un solo call site, `backgroundDecoration()` devolvía `null` en los seis tipos y `background()` devolvía lo mismo en los seis. Lo visible eran 6 `primary()` y 3 degradados, no las 46 referencias que decía esta nota. Ahora el acento es `AppColors.brandOf(context)` en todos los tipos (lo que pide COLOR_SYSTEM.md § "Serie activa": `brand` sólido). Sobrevive **solo** el modo intenso del fartlek, porque no es decoración por tipo sino color-como-estado —la misma idea por la que el descanso vira a azul— y sale del token `AppColors.effort`. El fichero pasó de 6 clases a 2.

    ⚠️ **El hallazgo de verdad — las zonas de FC se pintaban de 8 formas distintas.** Había una fuente canónica (`ZonesService._colors`, que usa `FcZoneBars`) y **8 copias ad-hoc** que no coincidían:
    - En el editor de bloques y en el de segmentos, **Z1 y Z2 estaban intercambiadas**: elegías "Z2" viendo azul y luego, corriendo, esa misma Z2 se pintaba verde.
    - La pantalla de sesión activa (`fc_widget`) tenía su **paleta entera distinta** en Z2-Z5.
    - `training_session_view` pintaba Z1 en teal `0xFF5A9E9E`, que parece un dedazo de `rpeLow` (`0xFF5A9E5A`).
    - El chip de FC% de `blocks_list_section` iba **desplazado una zona entera**: al 90%+ (Z5, máximo esfuerzo) pintaba coral en vez de rojo.

    Fuente única ahora: `ZonesService.colorForZone(zone)` y `ZonesService.colorForPercent(pct)`. **Cualquier sitio que pinte una zona sale de ahí** — lo vigila `test/unit/zones_color_test.dart`, que además busca por hex las paletas paralelas eliminadas (ese test encontró 2 de las 8 copias, que no se llamaban `_zoneColor` y se le habían escapado al grep).

    Sí se conservan hardcodeados los dos hex del descanso (`0xFFE3F2FD` tinte, `0xFF90CAF9` burbujas): COLOR_SYSTEM.md § "Descanso" los especifica literalmente.

    ⚠️ **Sin verificar en dispositivo.** Cambia el color de acento de las 6 pantallas de sesión y de los chips de zona en editor, home y analytics. Compila y la suite pasa, pero nadie lo ha visto corriendo.
