...
7. **Desplegar `deleteUserData`** — la Cloud Function existe (`functions/src/deleteUserData.ts`) y el cliente la llama con fallback al borrado parcial antiguo mientras no esté desplegada. Ejecutar `cd functions && npm run deploy` (o `firebase deploy --only functions:deleteUserData`).
8. **`_getWeekNumber()` en `home_flagship_chart.dart`** — aproximación de semana ISO que puede dar "Sem 0"/"Sem 53" en el cambio de año (solo etiquetas del gráfico 6M, no double-counting).
9. **Convención `fecha` (string ISO UTC)** — el esquema se mantiene (migrar a `Timestamp` requeriría backfill + actualizar Wear OS). Regla: cualquier query sobre `fecha` debe construir sus bounds con `.toUtc().toIso8601String()`, y cualquier bucketing por día/mes debe hacer `.toLocal()` tras el parse (ver `home_estadistica_repository.dart`).
