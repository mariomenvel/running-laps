...
9. **`firestore.rules` — crear grupos y unirse a retos está roto para todo el mundo** (descubierto jul 2026, bug presente desde el commit `cb7ea42` de abril 2026). `isSafeWrite()` y dos copias inline usan `request.resource.data.toString().size() < N` — `.toString()` no existe en el lenguaje de reglas de Firestore; al evaluar una función inexistente, Firestore deniega la escritura (fail-closed). Verificado con una prueba real (usuario de Auth genuino, no Admin SDK): `create` en `users/{uid}` (sin `.toString()`) → 200 OK; `create` en `groups/{groupId}` (con `.toString()`) → 403 `PERMISSION_DENIED`. Bloquea 3 operaciones para el 100% de los usuarios: crear grupo (línea ~221), unirse a un reto de grupo (línea ~284), unirse a un reto global (línea ~454). Ha pasado desapercibido porque Grupos está fuera del MVP. Fix (no aplicado a petición del usuario, jul 2026 — Grupos no es prioridad de lanzamiento): quitar la cláusula `.toString().size() < N` en las 3 reglas y dejar solo el `keys().size() < N` ya presente, que sí funciona.
